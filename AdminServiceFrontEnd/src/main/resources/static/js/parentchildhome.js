...
function getUserid() {
  ...
  function getUsertype() {
    const currentUrl = window.location.pathname;
    const usertype = currentUrl.split("/")[3];
    return usertype-1;
  }
...
async function saveUserInMongo(payload) {
  try {
    ...
    if (result.status === "Success") {
      ...
      } else if (result.message === "Not Authorized to Create this type of User") {
        alert("Not Authorized to Create this type of User !");
      }
    }
  } catch (error) {
...
async function getAllChildAndSetListeners(
  id,
  usertype,
  currentPage,
  itemsPerPage
...
function appendUserTypeButtons(buttonsArray) {
  const userTypeButtonsContainer = document.getElementById(
