...
async function setOwnerData() {
  const data =  await getUserIdAndUserType(); 
  if (data) {
...
async function setHyperMessage() {
  const response = await fetch(`http://localhost:7074/exuser/allHyperMessage`);
  const result = await response.json();
...
async function saveUser() {
  ...
  let repeatPasswordErrorText = document.getElementById(
  ...
  if (password !== repeatPassword) {
    ...
    let ownerData = await getUserIdAndUserType();
    const data = {
      id: `${ownerData.id}`,
...
async function saveUserInMongo(payload) {
  try {
    const response = await fetch(
      "http://localhost:7074/exuser/validateUserCreation",
      {
        method: "POST",
        headers: {
    ...
    if (result.status === "Success") {
      ...
      if (result.message === "WebsiteName Must be Required") {
        ...
        result.message ===
        "Password Must contains 1 Upper Case, 1 Lower Case & 1 Numeric Value & in Between 8-15 Charachter"
      ) {
        passwordErrorText.innerHTML =
          "Password Must contains 1 Upper Case, 1 Lower Case & 1 Numeric Value & in Between 8-15 Charachter !";
      ...
      } else if (result.message === "username Must be Required") {
        firstNameErrorText.innerHTML = "Firstname Must be Required !";
      }
    }
  } catch (error) {
...
async function getAllChild(id, usertype, currentPage, itemsPerPage) {
  const response = await fetch(
    `http://localhost:7074/exuser/${id}/${usertype}?pageNumber=${currentPage}&pageSize=${itemsPerPage}`
  );
...
function showAllChild(data) {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    childs.innerHTML += `
      ...
      <span class="lv_4" style="background:#568BC8; padding:1px;">DIR</span><span style="cursor:pointer;" id="userDataLink" data-userid="${
        child.id
      }" data-usertype="${child.usertype}" class="user-link">${
      child.userid
    }</span>
      </td>
      <td class="credit-amount-member">
        <span style="cursor:pointer; color: #0d6efd;" id="creditRefBtn" class="favor-set">${
          child.fixLimit
        }<i class="fas fa-pen ms-1"></i></span>
      </td>
      <td id="balance1">
        <a href="#" class="link-open">${
          child.myBalance + child.exposureLimit
        }</a>
      </td>
      <td style="color: red">
        <span style="cursor: pointer;width: 67px;text-align: center; display: inline-block;"class="status-suspend">${
          child.exposureLimit
        }</span>
      </td>
      ...
      <td id="refPL1">${
        child.myBalance + child.exposureLimit - child.fixLimit
      }</td>
      <td id="statusCol">
...
async function setPageListeners() {
  ...
  containers.forEach((row, i) => {
    ...
    const currentBalance = parseFloat(
      row.querySelector("#creditRefBtn").textContent
    );
...
async function showSettingPopup(userid, statusOfUser) {
  ...
  submitBtn.addEventListener("click", async function () {
    ...
    if (password !== "") {
      ...
      try {
        const response = await fetch(
          `http://localhost:7074/exuser/action/${action}`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
...
async function showPopup(currentBalance, userid, onPopupDisplay) {
  ...
  logbutton.addEventListener("click", function () {
    ...
    window.open(
      updatedUrl,
      "mywindow",
      "menubar=1,resizable=1,width='50%',height='80vh'"
    );
  });
  ...
  submitBtn.addEventListener("click", async function () {
    ...
    if (newValue !== "" && password !== "") {
      ...
      try {
        const response = await fetch(
          "http://localhost:7074/exuser/creditReference",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
...
async function showImpMessage() {
  const response = await fetch(
    "http://localhost:7074/exuser/currentImportantMessage"
  );
  const result = await response.json();
  const data = JSON.parse(decryptMessage(result.data));
...
async function searchUser(currentPage, itemsPerPage, userId) {
  ...
  const response = await fetch(
    `http://localhost:7074/exuser/search/${id}/${usertype}?pageNumber=${currentPage}&pageSize=${itemsPerPage}&userid=${userId}`
  );
  const result = await response.json();
  const encryptedData = result.data;
  var decryptData = JSON.parse(decryptMessage(encryptedData));
...
async function asdfgh(){
  setInterval(async () =>{
    const response = await fetch(
      "http://localhost:7074/exuser/data/6527e832a78c3962dd29f996"
    );
    const result = await response.json();
    const sessionId="6527e832a78c3962dd29f99";
    console.log(document.cookie, result.sessionId);
